...
public class SelectionComponent extends JComponent implements MouseListener, MouseMotionListener,
    ...
    private void paintSelectionPerimeter(Graphics g, List<PolyLine> segments) {
        ...
        for (PolyLine segment : segments) {
            int[] xPoints = segment.xs();
            int[] yPoints = segment.ys();

            g.drawPolyline(xPoints, yPoints, segment.size());
        }
    ...
    private void paintLiveWire(Graphics g) {
        ...
        if (!model.controlPoints.isEmpty()) {
            Point lastPoint = model.controlPoints.getLast();
            g.drawLine(lastPoint.x, lastPoint.y, mouseLocation.x, mouseLocation.y);
        }
    ...
    private void paintControlPoints(Graphics g, List<Point> points) {
        ...
        for (Point p : points) {
            g.fillOval(p.x - controlPointRadius, p.y - controlPointRadius, 2 * controlPointRadius,
                    2 * controlPointRadius);
        }
    }
    ...
    private void paintMoveGuides(Graphics g, List<Point> controlPoints) {
        ...
        if (selectedIndex < 0 || selectedIndex >= controlPoints.size()) {
            throw new IllegalArgumentException("Invalid selectedIndex: " + selectedIndex);
        }

        // Get the selected control point
        Point selectedPoint = controlPoints.get(selectedIndex);

        // Get the previous and next control points (wrap around using modulus)
        int prevIndex = (selectedIndex - 1 + controlPoints.size()) % controlPoints.size();
        int nextIndex = (selectedIndex + 1) % controlPoints.size();
        Point prevPoint = controlPoints.get(prevIndex);
        Point nextPoint = controlPoints.get(nextIndex);

        // Get the current mouse position
        Point mousePosition = getMousePosition(); // Assuming getMousePosition() gives current mouse location
        if (mousePosition == null) {
            return; // If the mouse position is unavailable, we return early.
        }

        // Set the graphics color for the live wire effect
        g.setColor(liveWireColor); // Assuming live wire color is red, replace with actual color if needed

        // Draw lines from mouse to the previous control point and next control point
        g.drawLine(mousePosition.x, mousePosition.y, prevPoint.x, prevPoint.y); // Line to previous point
        g.drawLine(mousePosition.x, mousePosition.y, nextPoint.x, nextPoint.y); // Line to next point
    }
    ...
    public void mousePressed(MouseEvent e) {
        ...
        if (e.getButton() == MouseEvent.BUTTON1 && model.state().isFinished()) {
            int mouseX = e.getX();
            int mouseY = e.getY();
            Point mousePoint = new Point(mouseX, mouseY);

            int closestIndex = model.closestPoint(mousePoint, controlPointRadius);
